...
const AboutAdinoPartnersBottomWhySection = ({ whyData }) => {
  if (!whyData) {
    return (
  ...
  const data =
    whyData?.data?.attributes ||
    whyData?.data ||
    whyData?.attributes ||
    whyData;

  const heading = data?.whyHeading2 || "";
  const description = data?.whyDescription2 || "";

  const itemsRaw = data?.whySectionItems;
...
export default AboutAdinoPartnersBottomWhySection;
