...
const AdinoPartnersServiceHero = ({ heroData }) => {
  if (!heroData) {
    return (
  ...
  const headingText = heroData?.heading_text ?? "";
  const heroBottomText = heroData?.hero_bottom_text?.[0]?.children?.[0]?.text ?? "";
  const heroBgUrl = heroData?.hero_bg?.url
    ? getStrapiMedia(heroData.hero_bg.url)
...
export default AdinoPartnersServiceHero;
